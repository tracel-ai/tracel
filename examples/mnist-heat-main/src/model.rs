...
// This is automatically generated by the derive macro
...
/// This trait is used to associate the `ModelTrainGroup` with the `Model` struct using a hash of the group name.
#[diagnostic::do_not_recommend]
impl<B: AutodiffBackend> HashedAssociatedType<{ train_group_name_hash("ModelTrainGroup") }>
    for Model<B>
{
...
// Other train groups can be defined similarly, e.g.:
// #[derive(Debug, Clone, Default)]
// pub struct ModelTrainGroup2<B: AutodiffBackend> {
//     _phantom: PhantomData<B>,
// }
...
impl<B: AutodiffBackend> TrainableModule<B> for Model<B> {
    fn all_train_fn_names() -> Vec<String> {
        <Self as HashedAssociatedType<{ train_group_name_hash("ModelTrainGroup") }>>::Inner::new()
            .all_train_fn_names()
    }
...
mod tests {
    ...
    type TG =
        <Model<B> as HashedAssociatedType<{ train_group_name_hash("ModelTrainGroup") }>>::Inner;
...
mod test_user_code {
    //! Note how this module does not need to import the type of the TrainGroup implementors, like `ModelTrainGroup`.
    //! This is because the type is hidden behind a clever use of the `HashedAssociatedType` trait, which allows us to refer to the train group by a constant.
    use crate::model::api::{HashedAssociatedType, TrainableModule};
    use crate::model::Model;
    ...
    mod train_group_impl {
        ...
        #[diagnostic::do_not_recommend]
        impl<B: AutodiffBackend> TrainGroup<B> for <Model<B> as HashedAssociatedType<{ train_group_name_hash("ModelTrainGroup") }>>::Inner {
            ...
            fn call_train_fn(
                ...
                match name {
                    "train1" => crate::model::test_user_code::trigger(Model::<B>::train1, context),
                    "train2" => crate::model::test_user_code::trigger(Model::<B>::train2, context),
