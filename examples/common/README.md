...
`modules()` directly constructs the experiment and inference modules for the backend selected by
the `TRACEL_CONNECTION` environment variable, so an example runs offline or against the cloud
