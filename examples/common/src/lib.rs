...
///
/// This is the pattern to copy into a real application: resolve the backend once, from the
...
pub fn context() -> anyhow::Result<Context> {
    match std::env::var("TRACEL_CONNECTION").as_deref() {
        Err(_) | Ok("offline") => Ok(Context::new(LocalBackend::new("./runs".into()))?),
        Ok("cloud") => Ok(Context::new(CloudBackend::new(AuthMethod::Env)?)?),
        Ok("station") => Ok(Context::new(StationBackend::new(station_url()?)?)?),
        Ok(other) => {
            anyhow::bail!(
