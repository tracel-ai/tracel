...
///
/// Large downloads are fetched as concurrent ranges when the source serves them, so throughput
/// is bounded by the network rather than by one connection. Each request's deadline follows the
/// size it moves, since a transfer's duration is set by the caller's bandwidth rather than by
/// any fixed budget.
#[derive(Clone)]
...
impl HttpTransferClient {
    /// Builds a client with the default request settings.
    pub fn new() -> Self {
