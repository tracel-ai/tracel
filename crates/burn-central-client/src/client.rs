...
mod tests {
    ...
    #[test]
    fn test_burn_central_builder() -> Result<(), BurnCentralError> {
        let credentials = BurnCentralCredentials::new("a");
        let central = BurnCentral::login(credentials).expect("Failed to login to Burn Central");

        assert!(central.me().is_ok());
