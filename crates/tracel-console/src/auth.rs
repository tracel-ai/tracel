...
use crate::{ConsoleError, normalize_base_url};
    ...
    pub fn poll_until_approved(
        ...
        loop {
            ...
            match self.poll(client_id, code)? {
                ...
                DevicePoll::Denied => return Err(DeviceAuthorizationError::Denied.into()),
                DevicePoll::Expired => return Err(DeviceAuthorizationError::Expired.into()),
            }
        }
    }
...
fn session_token_from_client(token: SessionToken) -> Result<SessionToken, ConsoleError> {
...
mod tests {
    ...
    #[test]
    fn the_device_secret_never_reaches_debug_output() {
    ...
    fn api_key_exchange_extracts_only_the_session_cookie_value() {
        ...
        assert_eq!(token.as_str(), "session-2");
    }
}
