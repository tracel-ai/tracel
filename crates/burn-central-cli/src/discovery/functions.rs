...
pub(crate) fn get_flags() -> Vec<FunctionMetadata> {
    FunctionRegistry::new().get_flags().to_vec()
}

pub fn get_registered_functions(flags: &[FunctionMetadata]) -> Vec<RegisteredFunction> {
    flags
        .iter()
        .map(|flag| {
            // function token stream to readable string
