...
mod targets {
    ...
    pub fn resolve_examples(
        ...
        let mut inferred =
            || infer_from_directory(package_root, Path::new(DEFAULT_EXAMPLE_DIR_NAME));
            ...
            Some(targets) => {
                ...
                for target in inferred_targets {
                    if !seen_names.contains(&target.name)
                        && !seen_paths.contains(&target_path(&target))
                    {
...
https://github.com/rust-lang/cargo/issues/5330",
    ...
    fn validate_unique_names(targets: &[TomlTarget], target_kind: &str) -> CargoResult<()> {
        ...
        for name in targets.iter().map(name_or_panic) {
            if !seen.insert(name) {
                anyhow::bail!(
